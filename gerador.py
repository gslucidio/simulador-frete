...
def gerar_excel(template_bytes, p):
    ...
    def clear_rows(ws, from_row):
        for r in range(from_row, max(ws.max_row, from_row)+1):
            for col in range(1, ws.max_column+1): sc(ws, r, col, None)

    ws_d["I3"].value=p["pc"]; ws_d["I4"].value=p["pv"]; ws_d["I8"].value=p["orig"]
    ws_d["I9"].value=p["p18"]; ws_d["I10"].value=p["p20"]; ws_d["I11"].value=p["p22"]
    ws_d["D14"].value=p["pl"]; ws_d["C10"].value=p["pct_sr"]; ws_d["C11"].value=p["pct_mz"]
    ws_d["C12"].value=round(1-p["pct_sr"]-p["pct_mz"],4)
    ws_d["D17"].value=p["cdi"]; ws_d["D18"].value=p["sr_sp"]; ws_d["D19"].value=p["mz_sp"]
    ws_d["I14"].value=p["cons"]; ws_d["I15"].value=p["gest"]; ws_d["J15"].value=p["piso_g"]
    ws_d["I16"].value=p["adm"]; ws_d["J16"].value=p["piso_a"]; ws_d["I17"].value=p["perf"]
    ...
    ws_f["X2"].value=f"=IRR(X5:X{LR},0.001)"; ws_f["AD2"].value=f"=IRR(AD5:AD{LR})"
    ws_f["AJ2"].value=f"=IRR(AJ5:AJ{LR},0.001)"; ws_f["AN2"].value=f"=IRR(AN5:AN{LR})"

    clear_rows(ws_f, 5); clear_rows(ws_c, 5)

    for n in range(1, N+1):
        r=n+4; pr=r-1; first=(n==1); last=(n==N); wind=(r==WIND); q_eim=(r>=WIND)
        sc(ws_f,r,2,"=Dashboard!D14" if first else f"=B{pr}*(((1+Dashboard!$C$20)^(1/30)))")
        sc(ws_f,r,3,n); sc(ws_f,r,4,START if first else f"=D{pr}+1")
        if first:
        ...
        sc(ws_c,r,2,n); sc(ws_c,r,3,START if first else f"=C{pr}+1")
        ...
        sc(ws_c,r,8,0 if first else f"=MAX(0,(Fundo!V{pr}+MAX(0,Fundo!T{pr})-Fundo!W{pr}*((1+Dashboard!$D$20)^(1/360)-1))*Dashboard!$I$17)")
...
def simular(p):
    N=p["n_dias"]; ORIG=N-21; pc=p["pc"]; pv=p["pv"]
    p18=p["p18"]; p20=p["p20"]; p22=p["p22"]
    pl=p["pl"]; sr0=pl*p["pct_sr"]; mz0=pl*p["pct_mz"]
    jr0=pl*round(1-p["pct_sr"]-p["pct_mz"],4)
    cdi_m=(1+p["cdi"])**(1/12)-1; sr_m=(1+p["sr_sp"])**(1/12)-1; mz_m=(1+p["mz_sp"])**(1/12)-1
    C20=(1+cdi_m)*(1+sr_m)-1; C21=(1+cdi_m)*(1+mz_m)-1
    cdi_d=(1+cdi_m)**(1/30)-1; sr_d=(1+C20)**(1/30)-1; mz_d=(1+C21)**(1/30)-1
    perf_d=(1+C20)**(1/360)-1  # hurdle = Hurdle Sr (D20)
    piso_a=p["piso_a"]/30; piso_g=p["piso_g"]/30
    U=pl; sr=sr0; mz=mz0
    E_h,I_h,M_h,R_h=[],[],[],[]
    dias,U_h,W_h,sr_h,mz_h,jr_h=[],[],[],[],[],[]
    c_adm=c_gest=c_cons=c_perf=0

    for n in range(1,N+1):
        first=(n==1); Q=p["orig"] if n<=ORIG else 0
        ...
        receb=G+K+O_
        V=U*cdi_d if not first else 0; W=U+desemb
        adm=max(piso_a,(p["adm"]*W/360)/30) if not first else piso_a
        gest=max(piso_g,(p["gest"]*W/360)/30) if not first else piso_g
        T=receb-R_h[n-18] if n>=18 else 0
        cons=R_h[n-2]*p["cons"] if not first else 0
        perf=max(0,(V+max(0,T)-W*perf_d)*p["perf"]) if not first else 0
        outros=10000 if first else 0
        custo=adm+gest+cons+perf+outros
        c_adm+=adm; c_gest+=gest; c_cons+=cons; c_perf+=perf
        sr+=sr*sr_d; mz+=mz*mz_d
        U=U-desemb+receb+V-custo
        dias.append(n); U_h.append(U); W_h.append(W)
        sr_h.append(sr); mz_h.append(mz); jr_h.append(U-sr-mz)

    return {
        "df": pd.DataFrame({"dia":dias,"caixa":U_h,"pl_fundo":W_h,"pl_sr":sr_h,"pl_mz":mz_h,"pl_jr":jr_h}),
        "U_final":U, "am_sr":sr, "am_mz":mz, "am_jr":U-sr-mz,
        "sr0":sr0, "mz0":mz0, "jr0":jr0,
        "c_adm":c_adm, "c_gest":c_gest, "c_cons":c_cons, "c_perf":c_perf,
...
with st.sidebar:
    st.markdown("## 🚛 Modelo de Frete"); st.markdown("---")
    ...
    st.markdown('<div class="sec">Prazos</div>', unsafe_allow_html=True)
    ...
    if abs(p18+p20+p22-1.0) > 0.01: st.warning(f"⚠️ Soma = {p18+p20+p22:.0%}")
    st.markdown('<div class="sec">Estrutura</div>', unsafe_allow_html=True)
    ...
    cons   = st.number_input("Consultoria (% desemb)", value=1.0, step=0.1) / 100
    ...
    perf   = st.number_input("Performance (%)",   value=20.0, step=5.0) / 100
    st.markdown("---")
    tpl_file = st.file_uploader("Template .xlsx", type=["xlsx"])

params = dict(n_dias=n_dias, pc=pc, pv=pv, orig=orig, p18=p18, p20=p20, p22=p22,
              pl=pl, pct_sr=pct_sr, pct_mz=pct_mz,
              cdi=cdi, sr_sp=sr_sp, mz_sp=mz_sp,
              cons=cons, gest=gest, piso_g=piso_g, adm=adm, piso_a=piso_a, perf=perf)

res = simular(params); df = res["df"]
carteira_peak=res["carteira_peak"]; util=carteira_peak/pl
moic_f=res["U_final"]/pl; moic_jr=res["am_jr"]/res["jr0"] if res["jr0"]>0 else 0
moic_sr=res["am_sr"]/res["sr0"]; moic_mz=res["am_mz"]/res["mz0"]
custo_total=res["c_adm"]+res["c_gest"]+res["c_cons"]+res["c_perf"]+10000
...
st.markdown(f"**{n_dias} dias** · Orig. até dia {n_dias-21} · {orig} ops/dia · PL R$ {pl/1e6:.2f}M")
st.markdown("---")

if util > 1.0:
    ops_max = int(pl / (pc * (18*p18+20*p20+22*p22)))
    st.markdown(f'<div class="warn-box">⚠️ <b>Fundo superdimensionado:</b> carteira no pico = R$ {carteira_peak:,.0f} ({util:.1%} do PL). Reduza para {ops_max} ops/dia para evitar caixa negativo.</div>', unsafe_allow_html=True)
if moic_jr < 1.0:
    st.markdown(f'<div class="warn-box">⚠️ <b>Júnior negativo:</b> custos totais (R$ {custo_total:,.0f}) vs margem bruta (R$ {margem:,.0f}). Revise originações, custos ou estrutura do fundo.</div>', unsafe_allow_html=True)

def kpi(col, label, val, sub="", neg=False, warn=False):
    cls="kpi-neg" if neg else ("kpi-warn" if warn else "")
    col.markdown(f'<div class="kpi {cls}"><div class="kpi-label">{label}</div><div class="kpi-value">{val}</div><div class="kpi-sub">{sub}</div></div>', unsafe_allow_html=True)

c1,c2,c3,c4,c5,c6 = st.columns(6)
kpi(c1,"Margem/op",f"R$ {pv-pc:,.0f}",f"Taxa: {pv/pc-1:.2%}")
kpi(c2,"Utilização",f"{util:.1%}",f"Pico: R$ {carteira_peak/1e3:.0f}k",warn=util>1.0)
kpi(c3,"MOIC Fundo",f"{moic_f:.3f}x",f"R$ {res['U_final']/1e3:.0f}k final",neg=moic_f<1)
kpi(c4,"MOIC Sênior",f"{moic_sr:.3f}x",f"Hurdle: CDI+{sr_sp:.0%}",neg=moic_sr<1)
...
PLOT = dict(plot_bgcolor="#0a0c12",paper_bgcolor="#0a0c12",
...
fig1=go.Figure()
fig1.add_trace(go.Scatter(x=df.dia,y=df.pl_sr/1e3,name="Sênior",line=dict(color="#4a9eff",width=2)))
fig1.add_trace(go.Scatter(x=df.dia,y=df.pl_mz/1e3,name="Mezanino",line=dict(color="#f5a623",width=2)))
fig1.add_trace(go.Scatter(x=df.dia,y=df.pl_jr/1e3,name="Júnior",line=dict(color="#e05c5c",width=2),
    fill="tozeroy",fillcolor="rgba(224,92,92,0.08)"))
fig1.add_hline(y=0,line_dash="dash",line_color="#444",line_width=1)
fig1.update_layout(**PLOT,height=300,xaxis=dict(title="Dia",gridcolor="#1a1d2a"),
...
with ca:
    ...
    fig2=go.Figure()
    fig2.add_trace(go.Scatter(x=df.dia,y=df.caixa/1e3,fill="tozeroy",
        fillcolor="rgba(0,212,170,0.08)",line=dict(color="#00d4aa",width=2),name="Caixa"))
    fig2.add_hline(y=0,line_dash="dash",line_color="#e05c5c",line_width=1)
    fig2.update_layout(**PLOT,height=270,showlegend=False,
        xaxis=dict(title="Dia",gridcolor="#1a1d2a"),yaxis=dict(title="R$ mil",gridcolor="#1a1d2a"))
    st.plotly_chart(fig2,use_container_width=True)

with cb:
    st.markdown('<div class="sec">Decomposição de custos</div>', unsafe_allow_html=True)
    fig3=go.Figure(go.Bar(
        x=["Adm","Gestão","Consultoria","Performance","Outros"],
        y=[res["c_adm"],res["c_gest"],res["c_cons"],res["c_perf"],10000],
        marker_color=["#4a9eff","#00d4aa","#f5a623","#e05c5c","#888"],
        text=[f"R${v/1e3:.0f}k" for v in [res["c_adm"],res["c_gest"],res["c_cons"],res["c_perf"],10000]],
        textposition="outside",textfont=dict(family="IBM Plex Mono",size=11)))
    fig3.add_hline(y=margem,line_dash="dash",line_color="#00d4aa",
        annotation_text=f"Margem bruta: R$ {margem/1e3:.0f}k",
        annotation_font_color="#00d4aa",annotation_position="top right")
    fig3.update_layout(**PLOT,height=270,showlegend=False,
        xaxis=dict(gridcolor="#1a1d2a"),yaxis=dict(title="R$",gridcolor="#1a1d2a"))
...
tbl=pd.DataFrame({
    ...
    "Entrada": [f"R$ {res['sr0']:,.0f}",f"R$ {res['mz0']:,.0f}",f"R$ {res['jr0']:,.0f}",f"R$ {pl:,.0f}"],
    "Saída":   [f"R$ {res['am_sr']:,.0f}",f"R$ {res['am_mz']:,.0f}",f"R$ {res['am_jr']:,.0f}",f"R$ {res['U_final']:,.0f}"],
    "MOIC":    [f"{moic_sr:.3f}x",f"{moic_mz:.3f}x",f"{moic_jr:.3f}x",f"{moic_f:.3f}x"],
    "Ganho":   [f"R$ {res['am_sr']-res['sr0']:+,.0f}",f"R$ {res['am_mz']-res['mz0']:+,.0f}",
                f"R$ {res['am_jr']-res['jr0']:+,.0f}",f"R$ {res['U_final']-pl:+,.0f}"],
})
...
tpl_bytes=None
if tpl_file: tpl_bytes=tpl_file.read()
else:
    try:
        with open("Modelagem_Frete_v5.xlsx","rb") as f: tpl_bytes=f.read()
    except FileNotFoundError:
        st.info("📁 Suba o `Modelagem_Frete_v5.xlsx` junto com o app no GitHub, ou use o upload na sidebar.")
if tpl_bytes:
    with st.spinner("Gerando Excel..."): xl=gerar_excel(tpl_bytes,params)
    st.download_button(label=f"⬇ Baixar Modelagem_Frete_{n_dias}d.xlsx",data=xl,
        file_name=f"Modelagem_Frete_{n_dias}d.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

with st.expander("Ver dados diários"):
    df_s=df.copy()
